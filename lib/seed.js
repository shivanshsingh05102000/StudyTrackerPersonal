...
function normalizeTopicResources(resources = {}) {
  const normalized = Object.fromEntries(
    Object.entries(resources || {}).map(([key, value]) => [key, normalizeStatus(value)])
  );
...
function buildInitialState(seedPath = SEED_PATH, now = new Date()) {
    ...
    for (const task of day.tasks || []) {
      if (task.id && task.topic) {
        const topic = {
          ...
          topic: task.topic,
          resources: normalizeTopicResources(task.resources),
          status: "pending",
  ...
  const state = {
    ...
    config: {
      ...
      minutesPerResource: { video: 45, pdf: 25, mcq: 20, notes: 15 },
      learnerName: "Learner",
      credentials: {
