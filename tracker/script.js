...
function loadState() {
  const defaults = { sessions: [], questions: [], timetable: structuredClone(defaultSlots), mode: 'class', timer: null };
  try {
...
const domainLabel = { afcat: 'AFCAT', dsa: 'DSA', tech: 'Tech', break: 'Break' };
...
function setPage(page) {
  ...
  $('page-title').textContent = { dashboard: 'Dashboard', timetable: 'Live Timetable', afcat: 'AFCAT Planner', dsa: 'DSA 45 Days', analytics: 'Analytics' }[page];
  render();
}

function render() {
  renderChrome(); renderDashboard(); renderTimetable(); renderAfcat(); renderDsa(); renderAnalytics(); renderTimer();
}
...
function importData(file) {
  ...
  reader.onload = () => { state = { ...loadState(), ...JSON.parse(reader.result) }; save(); render(); };
...
function init() {
  ...
  $('start-suggested-btn').addEventListener('click', () => { const s = (state.timetable[state.mode] || []).find((x) => x.domain !== 'break'); if (s) startFromSlot(s.id); });
  ...
  $('reset-btn').addEventListener('click', () => { if (confirm('Reset all local tracker data?')) { localStorage.removeItem(STORAGE_KEY); state = loadState(); render(); } });
  $('clear-today-btn').addEventListener('click', () => { state.sessions = state.sessions.filter((s) => s.date !== todayKey()); save(); render(); });
